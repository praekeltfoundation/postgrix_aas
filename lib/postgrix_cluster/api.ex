defmodule PostgrixCluster.API do
  ...
  def createDatabase(pid, db_name) do
    Postgrex.query!(pid, "set transaction isolation level repeatable read", [])
  ...
  def dropVaultRole(pid, db_name, vault_user) do
    Postgrex.query!(pid, "REVOKE ALL PRIVILEGES ON ALL TABLES IN SCHEMA public from #{vault_user};", [])
    Postgrex.query!(pid, "REVOKE ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public from #{vault_user};", [])
    Postgrex.query!(pid, "REVOKE ALL PRIVILEGES ON SCHEMA public from #{vault_user};", [])
    Postgrex.query!(pid, "REVOKE ALL PRIVILEGES ON DATABASE #{db_name} from #{vault_user};", [])
    Postgrex.query!(pid, "DROP ROLE IF EXISTS #{vault_user}", [])
  end
  ...
  def addOwnerRole(pid, db_name, db_owner, owner_password) do
    Postgrex.query!(pid, "CREATE ROLE #{db_owner} WITH LOGIN ENCRYPTED PASSWORD \'#{owner_password}\';", [])
    Postgrex.query!(pid, "ALTER DATABASE #{db_name} OWNER TO #{db_owner};", [])
    Postgrex.query!(pid, "ALTER SCHEMA public OWNER TO #{db_owner};", [])
  end
