defmodule Vault.API do
  ...
  def equipBindPolicy(policy_name, role_name) do
    ...
    Vaultix.Client.write(path, policy, :token, {Application.get_env(:postgrix_aas, Vault, :token)[:token]})
  end
  ...
  def addVaultPolicy(policy_name, role_name) do
    ...
    Vaultix.Client.write(path, payload, :token, {Application.get_env(:postgrix_aas, Vault, :token)[:token]})
  end
  ...
  def provision(host, port, db_name, vault_user, vault_pass, owner, default_ttl, max_ttl) do
    ...
    with {:ok, result} <- addDatabase(conn_name, host, port, db_name, vault_user, vault_pass, allowed_roles),
  ...
  def databaseExists?(conn_name) do
    path = "database/config/#{conn_name}"
    case Vaultix.Client.read(path, :token, {Application.get_env(:postgrix_aas, Vault, :token)[:token]}) do
    {:ok, result} -> true
    {:error, reason} -> false
    end
  end
  ...
  defp rollbackProvision(conn_name, role_name_r, role_name_rw, policy_name_r, policy_name_rw) do
    ...
    :ok <- deleteVaultPolicy(policy_name_rw) do
      ...
      _ -> raise RuntimeError, message: "Critical error: Failed to roll back a failed Vault provision."
    end
  end
